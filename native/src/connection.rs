...
use neon::prelude::*;
use redis::{parse_redis_url, Client, Connection, RedisError};
...
static mut REDIS_CONTEXT: RedisContext = RedisContext { connection: None };
...
fn init_connection(connection: &ConnectionOptions) -> Result<(), RedisError> {
  let mut connection_url = if connection.password.is_empty() == false {
    format!(
      "redis://{}:{}@{}",
      connection.user, connection.password, connection.host
    )
  } else {
